            ...
            <div v-if="showDetail[item.id]" class="detail">
                ...
                <div class="prooducts">
                    ...
                    <ul class="products_info">
                        <li>{{ item.purchases[0].brand_name }}</li>
                        <li>{{ item.purchases[0].product_name }}</li>
                        <li>{{ item.purchases[0].price }}</li>
                        <li>{{ item.purchases[0].size }}</li>
                        <li>{{ item.purchases[0].quantity }}</li>
                    </ul>
                </div>
            </div>
        </div>
...
const order_page_preparing = async () => {
    try {
        ...
        // 在這裡處理從後端獲取的資料
        jsonData.value = temp.filter((item) => item.purchases[0].purchase_status === 1)
